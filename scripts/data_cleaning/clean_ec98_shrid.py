...
def simplify_ec98_data(df):
    ...
    industry_emp_columns = [col for col in df_simplified.columns if col.startswith('ec98_emp_') and 'group' not in col and col not in core_columns]
    ...
    df_simplified['ec98_retail_diversity'] = (df_simplified[retail_columns] > 0).sum(axis=1)
    ...
    df_simplified['ec98_service_sophistication_score'] = (df_simplified[service_sophistication_columns] > 0).sum(axis=1)
    ...
    df_simplified['ec98_formal_employment_ratio'] = (
        (df_simplified['ec98_emp_gov'] + df_simplified['ec98_emp_priv']) / 
...
def save_simplified_data(df_simplified):
    ...
    summary_file = CLEANED_FILES_DIR / "ec98_shrid_summary_stats.csv"
